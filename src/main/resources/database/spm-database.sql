...
CREATE TABLE IF NOT EXISTS `spm`.`course` (
  ...
  INDEX `fk_course_teacher_idx` (`teacher_user_id` ASC),
...
CREATE TABLE IF NOT EXISTS `spm`.`experiment_file` (
  ...
  INDEX `fk_experiment_has_filesource_filesource1_idx` (`file_source_id` ASC),
  INDEX `fk_experiment_has_filesource_experiment1_idx` (`experiment_id` ASC),
...
CREATE TABLE IF NOT EXISTS `spm`.`section_file` (
  ...
  INDEX `fk_section_has_filesource_section1_idx` (`section_id` ASC),
  INDEX `fk_section_has_filesource_filesource1_idx` (`file_source_id` ASC),
...
CREATE TABLE IF NOT EXISTS `spm`.`student_course_experiment` (
  ...
  INDEX `fk_student_course_approve_has_experiment1_experiment1_idx` (`experiment_id` ASC),
  ...
  INDEX `fk_stu_course_exp_file2_idx` (`video_file` ASC),
...
CREATE TABLE IF NOT EXISTS `spm`.`token_secret` (
  ...
  UNIQUE INDEX `token_secret_id_UNIQUE` (`token_secret_id` ASC))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `spm`.`exam_score`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `spm`.`exam_score` ;

CREATE TABLE IF NOT EXISTS `spm`.`exam_score` (
  `exam_id` INT UNSIGNED NOT NULL,
  `selected_course_student_user_id` INT UNSIGNED NOT NULL,
  `selected_course_course_course_id` INT UNSIGNED NOT NULL,
  `exam_score` INT NULL,
  PRIMARY KEY (`exam_id`, `selected_course_student_user_id`, `selected_course_course_course_id`),
  INDEX `fk_exam_has_selected_course_selected_course1_idx` (`selected_course_student_user_id` ASC, `selected_course_course_course_id` ASC),
  INDEX `fk_exam_has_selected_course_exam1_idx` (`exam_id` ASC),
  CONSTRAINT `fk_exam_has_selected_course_exam1`
    FOREIGN KEY (`exam_id`)
    REFERENCES `spm`.`exam` (`exam_id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_exam_has_selected_course_selected_course1`
    FOREIGN KEY (`selected_course_student_user_id` , `selected_course_course_course_id`)
    REFERENCES `spm`.`selected_course` (`student_user_id` , `course_course_id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
